...
<div class="container mt-4">
    ...
    <h3>Edit Status Arsip</h3>

    <form action="{{ route('arsip_admin.update-admin', $id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="form-group mt-4">
          <label class="form-label-custom" for="status">
            <i class="bi bi-shield-lock"></i> Status <span class="required">*</span>
          </label>
          <div class="input-wrap">
            ...
            <select name="status" id="status" class="form-input @error('status') is-invalid @enderror" required>
              ...
              <option value="verify" {{ (old('status') ?? $data->status) == 'verify' ? 'selected' : '' }}>Verify</option>
              <option value="input" {{ (old('status') ?? $data->status) == 'input' ? 'selected' : '' }}>Input</option>
